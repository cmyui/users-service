...
async def test_should_not_create_account_with_invalid_username(ctx: Context):
    ...
    assert data is ServiceError.ACCOUNTS_USERNAME_INVALID
...
async def test_should_not_create_account_with_preexisting_username(ctx: Context):
    username = sample_data.fake_username()
    password = sample_data.fake_password()
    first_name = sample_data.fake_first_name()
    last_name = sample_data.fake_last_name()
    ...
    assert data2 is ServiceError.ACCOUNTS_USERNAME_EXISTS
...
async def test_should_fetch_one_account_by_username(ctx: Context):
    username = sample_data.fake_username()
    password = sample_data.fake_password()
    first_name = sample_data.fake_first_name()
    last_name = sample_data.fake_last_name()
    ...
    data2 = await accounts.fetch_one(ctx, username=username)
    assert not isinstance(data2, ServiceError)
    assert data2["username"] == username
    assert "password" not in data2
    assert data2["first_name"] == first_name
...
async def test_should_not_fetch_one_deleted_account_by_username(ctx: Context):
    username = sample_data.fake_username()
    password = sample_data.fake_password()
    first_name = sample_data.fake_first_name()
    last_name = sample_data.fake_last_name()
    ...
    data3 = await accounts.fetch_one(ctx, username=data["username"])
    assert data3 is ServiceError.ACCOUNTS_NOT_FOUND
